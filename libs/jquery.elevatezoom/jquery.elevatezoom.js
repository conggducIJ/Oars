if (typeof Object.create !== "function") {
    ...
    var c = {
        ...
        startZoom: function() {
            ...
            g.zoomContainer.add(g.$elem).mouseenter(function() { if (g.overWindow == false) { g.setElements("show") } }).mouseleave(function() { if (!g.scrollLock) { g.setElements("hide") } });
            if (g.options.zoomType != "inner") {
            ...
            if (g.options.responsive && !g.options.scrollZoom) {
                if (g.options.showLens) {
                    ...
                    if (g.options.zoomType != "lens") {
                        if (g.nzHeight < g.options.zoomWindowWidth / g.widthRatio) { lensHeight = g.nzHeight } else { lensHeight = String((g.options.zoomWindowHeight / g.heightRatio)) }
                        if (g.options.zoomWindowWidth < g.options.zoomWindowWidth) { lensWidth = g.nzWidth } else { lensWidth = (g.options.zoomWindowWidth / g.widthRatio) }
        ...
        showHideWindow: function(f) {
            ...
            if (f == "hide") {
                if (g.isWindowActive) {
                    if (g.options.zoomWindowFadeOut) { g.zoomWindow.stop(true, true).fadeOut(g.options.zoomWindowFadeOut) } else { g.zoomWindow.hide() }
                    g.isWindowActive = false
                }
            }
        },
        ...
        setWindowPostition: function(f) {
            ...
            if (g.options.zoomType == "window" || g.options.zoomType == "inner") {
                ...
                if (g.largeHeight < g.options.zoomWindowHeight) { g.windowTopPos = 0 }
